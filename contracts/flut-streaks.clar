...
(define-map streaks {user: principal} {interval: uint, last-deposit: uint, count: uint, total: uint})
...
(define-public (start-streak (amount uint) (interval uint))
  (begin
    ...
    (map-set streaks {user: tx-sender} {interval: interval, last-deposit: block-height, count: u1, total: amount})
    (ok true)))
  ...
  (match (map-get? streaks {user: tx-sender})
    streak (begin
      ...
      (let ((on-time (< block-height (+ (get last-deposit streak) (* (get interval streak) u2))))
        ...
        (map-set streaks {user: tx-sender} (merge streak {last-deposit: block-height, count: new-count, total: (+ (get total streak) amount)}))
        (ok new-count)))
    ERR-NO-STREAK))
