...
(define-map goals {goal-id: uint} {owner: principal, label: (string-ascii 60), target: uint, saved: uint, reached: bool})
...
(define-public (create-goal (label (string-ascii 60)) (target uint))
  (let ((id (var-get goal-counter)))
    (map-set goals {goal-id: id} {owner: tx-sender, label: label, target: target, saved: u0, reached: false})
...
(define-public (withdraw-goal (goal-id uint))
  (match (map-get? goals {goal-id: goal-id})
    goal (begin
      ...
      (asserts! (get reached goal) ERR-ALREADY-REACHED)
      (let ((caller tx-sender))
        (try! (as-contract (stx-transfer? (get saved goal) tx-sender caller))))
      (map-set goals {goal-id: goal-id} (merge goal {saved: u0}))
      (ok true))
    ERR-NOT-FOUND))
